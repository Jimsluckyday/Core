...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const scoreboardRes = await fetch(`https://site.api.espn.com/apis/site/v2/sports/basketball/wnba/scoreboard?dates=${espnDate}`);
    if (!scoreboardRes.ok) {
    ...
    const rosterResults = await Promise.allSettled(
      [...teamsToday.keys()].map(id =>
        fetch(`https://site.api.espn.com/apis/site/v2/sports/basketball/wnba/teams/${id}/roster`).then(r => r.ok ? r.json() : null)
      )
    );
